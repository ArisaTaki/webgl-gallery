...
try {
  ...
  if (status.ok !== true || status.configured !== false) {
    failures.push(`Expected first-run setup status from installed Docker package, got ${JSON.stringify(status)}.`);
  }
  ...
  const report = {
    ...
    installOutput: install.stdout.split('\n').filter(Boolean).slice(-8),
    failures,
  };
  ...
  if (!process.env.KEEP_DOCKER_INSTALL_QA_TMP) {
...
async function waitForStatus(baseUrl) {
  ...
  const logs = await run('docker', ['compose', 'logs', '--no-color'], {
    cwd: installDir,
    env,
  }).catch((error) => ({ stdout: '', stderr: String(error) }));
  throw new Error(`Timed out waiting for installed Docker package. Last error: ${lastError?.message || lastError}\n${logs.stdout}\n${logs.stderr}`);
}

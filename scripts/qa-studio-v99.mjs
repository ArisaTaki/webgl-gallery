...
async function staticChecks() {
  ...
  const required = [
    ...
    ['Studio loading state', main.includes('data-studio-loading') && styles.includes('@keyframes studio-loading-pulse')],
    ['album theme color', main.includes('name="accentColor"') && main.includes('createAlbumAccentPalette')],
    ['semantic gallery metadata', main.includes('data-meta-date') && main.includes('data-index-active-title') && main.includes('galleryProjectMeta')],
    ['album preload queue', main.includes('INITIAL_ALBUM_PRELOAD_COUNT') && main.includes('preloadImageQueue') && main.includes('initialAlbumPreloadUrls')],
    ['background image preload', main.includes('BACKGROUND_PRELOAD_CONCURRENCY') && main.includes('scheduleBackgroundImagePreload')],
    ['progressive work image', main.includes('getWorkPreviewSrc') && main.includes("img.dataset.quality = 'preview'")],
